...
//	  Int_t kLHCbFont        = 132;  // Old LHCb style: 62;
    ...
    else if (option.Contains("Belle2")) {
...
//  h_error->SetBit(kMustCleanup);
  ...
  legend->SetTextFont(kLHCbFont);
	...
	legend->SetHeader(Form("Run test: #it{p} = %.2f, #chi^{2}/ndf = %.2f (p = %.2f)",RunTest(pulls), chi2_reduced, chi2_pvalue));
...
void doocore::lutils::PlotGauss(TString pName, const TH1 & pulls, TString pDir, double chi2_reduced, double chi2_pvalue) {
  ...
  PlotPullDistributionWithGaussian(pulls, c1, f_gauss_norm, f_gauss_fit, h_pulls, h_error, legend, chi2_reduced, chi2_pvalue);
	printPlot(&c1, pName, pDir);
...
void doocore::lutils::PlotPulls(TString pName, RooPlot * pFrame, TLatex& label, TString pDir, bool plot_logy, bool plot_logx, std::string gauss_suffix, unsigned int num_fit_params) {
  ...
  double ndof         = pFrame->GetNbinsX()-num_fit_params;
  double chi2_pvalue  = TMath::Prob(chi2_reduced*ndof, ndof);
