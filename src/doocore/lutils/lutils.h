 ...
 */
void PlotPullDistributionWithGaussian(const TH1& pulls, TPad& pad, TF1* f_gauss_norm, TF1* f_gauss_fit, TH1* h_pulls, TH1* h_error, TLegend* legend, double chi2_reduced);

  
/**
 *  Plot a distribution histogram of the pulls overlaid with a Gaussian
 */
void PlotGauss(TString pName, const TH1 & pulls, TString pDir = "", double chi2_reduced = 0.0);
...
void PlotPulls(TString pName, RooPlot * pFrame, TLatex& label,
               ...
               std::string gauss_suffix="_Gauss", unsigned int num_degrees_freedom=0);
