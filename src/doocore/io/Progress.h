 ...
 private:
  ...
  void Update(bool force_update=false) {
    if ((tty_ && steps_since_update_ > step_position_update_tty_) || force_update) {
      ...
      printf("%s %.2f %% (el. / rem. / st.[ms]: %s / %s / %.2f)        \xd", MakeProgressBar(progress_fraction_).c_str(), (progress_fraction_*100.0), SecondsToTimeString(elapsed_).c_str(), SecondsToTimeString(remaining).c_str(), (elapsed_/position_*1000.0));
      fflush(stdout);
